...
def get_weather_current(city_name: str) -> str:
    ...
    if not WEATHER_API_KEY:
        return"Missing or invalid WEATHER_API_KEY"
    ...
    try:
        res = requests.get(BASE_CURRENT_URL, params=params)
...
def get_weather_forecast(city_name: str) -> str:
    ...
    try:
        res = requests.get(BASE_FORECAST_URL, params=params)
        res.raise_for_status()
        data = res.json()
        ...
        if res.status_code == 404:
            return f"[TOOL ERROR] City '{city_name}' not found. Please verify the spelling."
        return f"[TOOL ERROR] Forecast service returned status code {res.status_code}."
    except Exception as e:
        return f"[TOOL ERROR] Unexpected error: {str(e)}"